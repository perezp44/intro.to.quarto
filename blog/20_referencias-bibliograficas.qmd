...
Tenemos ya un fichero `.bib` con las referencias, ¿cómo incluimos citas a ellos en nuestro fichero `.qmd`? Podemos seguir escribiendo y gestionando estas citas a mano, pero si queremos hacerlo "automaticamente"^[En realidad las citas hay que escribirlas a mano, pero si las incluyes con la sintáxis adecuada, hará que cada cita tenga un enlace a la referencia] tenemos que saber cómo hacerlo.
